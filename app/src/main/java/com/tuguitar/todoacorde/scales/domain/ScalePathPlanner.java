...
public final class ScalePathPlanner {
    ...
    public List<ScaleFretNote> bindSequenceToPattern(List<String> expectedNotes,
        ...
        for (String targetRaw : expectedNotes) {
            ...
            // Candidatos: todas las posiciones de esa nota (enarmónicos incluidos)
            ...
            if (candidates.isEmpty()) {
                // Sin candidatos → repetimos la última si existe
            ...
            if (last == null) {
                ...
                // Seguridad: si por algún caso devolviese null, cae a Manhattan
                if (chosen == null) {
                    chosen = chooseByManhattan(last, candidates);
                }
                // Evitar duplicar EXACTAMENTE la misma posición
    ...
    private ScaleFretNote chooseNextAscending(ScaleFretNote last,
        ...
        // 1) Buscar solo candidatos con pitch estrictamente mayor
        for (ScaleFretNote cand : candidates) {
            ...
            if (p > lastPitch) {
                ...
                if (delta < bestDelta) {
                    ...
                    // Desempate: preferir cuerda más aguda (índice mayor); luego menor traste
        ...
        // 2) Si no hay pitch mayor (caso límite en la caja), intentamos mismo pitch (evitar estancamiento)
        ...
        for (ScaleFretNote cand : candidates) {
            ...
            if (p == lastPitch) {
                if (samePitch == null) samePitch = cand;
                    ...
                    // Desempate: mover hacia 1ª cuerda si es posible
        ...
        // 3) Fallback: Manhattan (debería ocurrir raramente)
    ...
    private ScaleFretNote chooseByManhattan(ScaleFretNote last, List<ScaleFretNote> candidates) {
        ...
        for (int i = 1; i < candidates.size(); i++) {
            ...
            if (d < bestDist) {
                ...
                // Desempate: empuja hacia 1ª cuerda y menor traste
